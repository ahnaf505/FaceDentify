...
def run_operation(operation_number):
    try:
        if operation_number == 1:  # Insert Single Face
            ...
            progress_bar["maximum"] = len(peoples)
            ...
            progress_bar["maximum"] = len(faces_dir)
            progress_bar["value"] = 0
            resultfound = False
            log_output("Searching db for similar faces...")
            for i, facedir in enumerate(faces_dir):
            ...
            for i, facedir in enumerate(faces_dir):
                if faceid == facedir[:-5]:
                    fullname = search_fullname(faceid)[0]['fullname']
                    log_output(f"Full Name: {fullname}")
                    log_output(f"Raw face image path: faceimgdb/{faceid}.png")
                    show_image_text_dialog(root, "Candidate Data", f"imgfacedb/{faceid}.png", f"Full Name: {fullname}")
                    break
                progress_bar["value"] = i + 1
                root.update_idletasks()
...
log_area = tk.Text(root, height=10, width=50, state="normal")
